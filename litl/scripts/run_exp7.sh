...
# echo "UTASCL "
# LD_LIBRARY_PATH=$LITLLIB_DIR:$LD_LIBRARY_PATH $LITL_DIR/bin/utascl_bench -t $thread -u $thread -T $time -S $short -s $SHORT_CRI -g $LONG_CRI -d $DELAYTIME > result
# $LOCAL_DIR/measure.sh ./result $thread $thread
# sleep 1
...
echo "UTASPC "
LD_LIBRARY_PATH=$LITLLIB_DIR:$LD_LIBRARY_PATH $LITL_DIR/bin/utaspc_bench -t $thread -u $thread -T $time -S $short -s $SHORT_CRI -g $LONG_CRI -d $DELAYTIME > result
$LOCAL_DIR/measure.sh ./result $thread $thread
sleep 1
rm result
