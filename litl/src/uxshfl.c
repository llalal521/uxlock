...
#define uxshfl_MIN_WAIT_TIME	50LL
#define uxshfl_MAX_WAIT_TIME	10000000LL
extern __thread unsigned int cur_thread_id;
extern __thread struct t_info tinfo;
extern unsigned int last_thread_id;
__thread int uxthread;
...
/* debugging */
/* #ifdef WAITER_DEBUG */
static inline void traverse_nodes(uxshfl_mutex_t *lock, uxshfl_node_t *node)
{
    uxshfl_node_t *curr = node;
    int count = 0;
...
#define _uxshfl_NOSTEAL_VAL        (1U << (_uxshfl_LOCKED_OFFSET + _uxshfl_LOCKED_BITS))
#define uxshfl_MAX_PATIENCE_COUNT  2
#define MAX_CONT_uxshflD_COUNT    2
...
static inline void enable_stealing(uxshfl_mutex_t *lock)
{
        atomic_andnot(_uxshfl_NOSTEAL_VAL, &lock->val);
}

static inline void disable_stealing(uxshfl_mutex_t *lock)
{
        atomic_fetch_or_acquire(_uxshfl_NOSTEAL_VAL, &lock->val);
}

static inline uint8_t is_stealing_disabled(uxshfl_mutex_t *lock)
{
        return READ_ONCE(lock->no_stealing);
}

static inline void set_sleader(struct uxshfl_node *node, struct uxshfl_node *qend)
{
...
static inline void clear_sleader(struct uxshfl_node *node)
{
        node->sleader = 0;
}

static inline void set_waitcount(struct uxshfl_node *node, int count)
{
...
#define USE_COUNTER
static void shuffle_waiters(uxshfl_mutex_t *lock, struct uxshfl_node *node,
                            int is_next_waiter)
{
    uxshfl_node_t *curr, *prev, *next, *last, *sleader, *qend;
    int nid = node->nid;
...
#ifdef USE_COUNTER
    if (curr_locked_count >= uxshfl_MAX_LOCK_COUNT) {
...
uxshfl_mutex_t *uxshfl_mutex_create(const pthread_mutexattr_t *attr) {
    uxshfl_mutex_t *impl = (uxshfl_mutex_t *)alloc_cache_align(sizeof(uxshfl_mutex_t));
    impl->tail = NULL;
    impl->val = 0;
#ifdef WAITER_CORRECTNESS
    impl->slocked = 0;
...
static int __uxshfl_mutex_lock(uxshfl_mutex_t *impl, uxshfl_node_t *me)
{
	uxshfl_node_t *prev;
    ...
    if (prev) {
        ...
        for (;;) {
            ...
            if (READ_ONCE(me->lstatus) == uxshfl_STATUS_LOCKED)
                break;
...
int uxshfl_mutex_lock(uxshfl_mutex_t *impl, uxshfl_node_t *me) {
    int ret = __uxshfl_mutex_lock(impl, me);
    assert(ret == 0);
#if COND_VAR
    if (ret == 0) {
...
static inline void __uxshfl_mutex_unlock(uxshfl_mutex_t *impl, uxshfl_node_t *me) {
#ifdef BLOCKING_FAIRNESS
...
int uxshfl_cond_wait(uxshfl_cond_t *cond, uxshfl_mutex_t *lock, uxshfl_node_t *me) {
    return uxshfl_cond_timedwait(cond, lock, me, 0);
}
...
void uxshfl_init_context(lock_mutex_t *UNUSED(impl),
