...
int access_variables(volatile uint64_t * memory_area, int number_of_variables)
{
	int i = 0;
...
int read_variables(volatile uint64_t * memory_area, int number_of_variables)
{
  	int i = 0;
        int ret = 0;

	for (i = 0; i < number_of_variables; i++) {
		ret += *(memory_area + 8 * i + 7) + 1;
		ret += *(memory_area + 8 * i + 2) + 1;
	}
        return ret;
}
...
void reader_cs(int tid)
{
	tt_startp = PAPI_get_real_cyc();
	pthread_rwlock_rdlock(&global_lock);
	tt_endp = PAPI_get_real_cyc();
	global_cnt[tid]++;
	read_variables(short_shared_variables_memory_area, short_number_of_shared_variables);
	pthread_rwlock_unlock(&global_lock);
}
...
int main(int argc, char *argv[])
{
	...
	while ((command = getopt(argc, argv, "m:g:u:s:p:d:t:hT:r:l:S:r:")) != -1) {
		switch (command) {
		case 'h':
			print_help();
			exit(0);
	...
	pthread_rwlock_init(&global_lock, 0);
	switch (mode) {
	case 0:
